...
public class StatusHandle extends AbstractHandle {
    ...
    public void handle(String[] commandArgs) throws Exception {
        ...
        if (commandArgs.length < 1) {
            System.err.println("用法: cvs " + tag + " status <目标路径>");
            System.err.println("  示例 (查看目录所有文件历史): cvs " + tag + " status /path/to/dir");
            System.err.println("  示例 (查看单个文件历史): cvs " + tag + " status /path/to/file.txt");
            return;
        }
